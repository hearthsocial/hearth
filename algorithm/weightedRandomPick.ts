...
export function weightedRandomPick(
  ...
  const maxPossibleLoops = usedTags.reduce((sum,tagData)=>sum+tagData.maxUses,0)
  if(maxPossibleLoops>repeat)console.error("requested number of posts was less than possible from current data.")
  ...
  while(steps<safeRepeat){
    ...
    Object.keys(weightedTags).find((tagName) => {//loops through object until eventually finding the range in which the random number lies
        positionInObject++
      const range = weightedTags[tagName];
      return rand >= range.start && rand < range.end;
    });
    let curTagData = usedTags[positionInObject]
    while(curTagData&&curTagData.timesUsed>=curTagData.maxUses){//if the selected tag has already maxed out, scroll through array to find the next available option
        positionInObject = positionInObject==usedTags.length?0:positionInObject++
        curTagData = usedTags[positionInObject]
    }
    if (curTagData) {//just to verify that we did find a real tag
