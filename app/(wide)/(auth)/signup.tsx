...
useEffect(()=>{
  ...
  async function createAccount(){
    ...
    const {error:accError} = await supabase.from("accounts").insert({username:username,id:userId})
    if(accError){
      setError("Signup failed. Please try again later, or contact support.")
    }
    ...
    <View style={styles.container}>
      <View style={styles.topBox}>
        ...
        {usernameState==2&&username!==""&&(
                <Text style={styles.failedUsername}>@{username} is not available.</Text>
        )}
        {usernameState==3&&username!==""&&(
                <Text style={styles.availableUsername}>@{username} is available!</Text>
        )}
        ...
        <View style={styles.or}>
         ...
         <View style={styles.orOptionsBox}>
            ...
            <Text onPress={()=>{router.replace("/(wide)/(auth)/login")}}style={styles.orOptionText}>Login</Text>
            <Text style={styles.separator}></Text>
            <Text style={styles.orOptionText} onPress={continueAsGuest}>View as guest</Text>
